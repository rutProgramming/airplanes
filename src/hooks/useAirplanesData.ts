...
//   // rows לפי bufferIds (זה ה-"window" שלך)
//   const rows = useMemo(() => {
...
//   return { rows, loading, hasMore, totalCount, loadNext, loadPrev };
// }
...
export function useAirplanesData(args: { filters: Filters; sort: Sort | null }) {
  ...
  // reset + initial fetch when filters/sort change
  useEffect(() => {
    dispatch(airplanesSlice.actions.resetView());
    dispatch(airplanesInitRequested({ filters: args.filters, sort: args.sort }));
    // scrollTop reset should be done in the component (because hook doesn't know containerRef)
  }, [dispatch, args.filters, args.sort]);
